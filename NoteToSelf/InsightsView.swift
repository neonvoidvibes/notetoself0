...
struct InsightsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Locked overlay if not subscribed
            if !subscriptionManager.isUserSubscribed {
                VStack {
                    Spacer()
                    Text("Advanced Analytics Locked")
                        .font(UIStyles.headingFont)
                        .foregroundColor(.white)
                        .padding()
                    
                    Text("Subscribe in Settings to unlock deeper insights.")
                        .font(UIStyles.bodyFont)
    ...
    // Simple streak calculation: number of consecutive days from the most recent entry
    ...
    var body: some View {
        ...
        LazyVGrid(columns: columns, spacing: 16) {
            ...
            ForEach(1...daysCount, id: \.self) { dayIndex in
                let realDate = Calendar.current.date(byAdding: .day, value: dayIndex - 1, to: start)!
...
struct CalendarHelper {
    static var calendar: Calendar = {
        ...
        cal.firstWeekday = 2 // Monday is 2
        return cal
    }()
